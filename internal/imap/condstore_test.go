...
func TestCondstoreUnchangedMailboxPublishesNoOp(t *testing.T) {
	...
	assert.NotContains(commands, "SELECT INBOX")
	assert.NotContains(commands, "UID SEARCH")
	assert.NotContains(commands, "UID FETCH")
}
...
func TestCondstoreAppendAndSameCountSwap(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal([]string{tt.wantListed}, listQresyncMessages(t, client))
			deltas := client.ObservedMailboxDeltas()
			require.Len(t, deltas, 1)
			assert.Equal(tt.wantVanished, deltas[0].VanishedUIDs)
			assert.Equal(uidsToUint32(tt.currentUIDs), deltas[0].State.KnownUIDs)
			assert.True(deltas[0].Incremental)
			commands := joinedCommands(server.commandsFor(1))
...
func TestCondstoreInvalidBaselineFallsBackToFullEnumeration(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NotContains(joinedCommands(server.commandsFor(1)), "CHANGEDSINCE")
		})
	}
}
...
func TestCondstoreIncompleteSearchCannotPublishVanishedUIDs(t *testing.T) {
	...
	assert.NotContains(joinedCommands(server.commandsFor(2)), "CHANGEDSINCE")
}
